...
def _(mo):
    ...
    mo.vstack(
        ...
        items=[
            ...
            mo.md(
                r"### This is an interactive notebook that applies [Eisenstein's criterion](https://en.wikipedia.org/wiki/Eisenstein's_criterion) to test a randomly generated polynomial $q(x)\in \mathbb{Z}[x]$ for [irreducibility](https://en.wikipedia.org/wiki/Irreducible_polynomial) over $\mathbb{Q}$"
            ),
            mo.md("# Eisenstein's Criterion Overview"),
            mo.md(
                r"### Let $q(x) = a_0 + a_1(x) + a_2(x^2) + ... + a_n(x^n)$, $a_i \in \mathbb{Z}$"
            ),
            mo.md(
                r"### Eisenstein's criterion states that $q(x)$ is irreducible over $\mathbb{Q}$ if there exists a prime number $p\in \mathbb{Z}$ such that all three conditions are met:"
            ),
